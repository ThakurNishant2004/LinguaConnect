...
const calculateAvgResponseTime = async (start, end) => {
...
// ---------------------- DASHBOARD STATS ----------------------
export const getDashboardStats = async (period = "24h") => {
    ...
    const calcChange = (current, prev) => prev === 0 ? 0 : +(((current - prev) / prev) * 100).toFixed(1);
    const stats = {
        totalConversations: {
...
export const addLanguage = async (language) => {
    ...
    if (exists)
        return { message: `Language '${language}' already exists.` };
...
// ---------------------- FAQ ----------------------
export const uploadFAQs = async (faqs) => {
